...
import PlateModal from './Components/BorderModal';
import EditPlateModal from './Components/EditBorderModal';
...
const Border = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/border`);
        console.log('Dados retornados:', response.data); // Adicione isso para depuração
        setData(response.data);
        setFilteredData(response.data);
      } catch (error) {
        console.error('Erro ao buscar as bordas:', error);
  ...
  const handleSave = async () => {
    try {
      ...
      console.error('Erro ao atualizar as bordas:', error);
    }
  };

  const handleEdit = (borderId) => {
    setSelectedBorderId(borderId);
    setEditModalVisible(true);
  };

  const handleDelete = (borderId) => {
    setSelectedBorderId(borderId);
