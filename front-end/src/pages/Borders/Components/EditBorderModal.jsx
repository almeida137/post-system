...
const EditBorderModal = ({ visible, onCancel, onSave, borderId }) => {
  const [form] = Form.useForm();
  const [initialValues, setInitialValues] = useState(null);

  useEffect(() => {
    if (borderId) {
      axios.get(`${import.meta.env.VITE_API_URL}/api/border/${borderId}`)
        .then(response => {
  ...
  const handleSave = () => {
    form.validateFields().then(values => {
      ...
      if (!isEqual(initialValues, updatedValues)) {
        axios.put(`${import.meta.env.VITE_API_URL}/api/border/${borderId}`, updatedValues)
          .then(() => {
            onSave();
...
export default EditBorderModal;
