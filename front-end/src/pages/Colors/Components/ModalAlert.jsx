...
const AlertModal = ({ title, visible, onCancel, onSave, brandId, colorId }) => {

  const handleConfirm = async (brandId, colorId) => {
    try {
      const response = await axios.delete(`${import.meta.env.VITE_API_URL}/api/color/${brandId}/colors/${colorId}`);
    ...
    <StyledModal
      ...
      footer={
        <CustomFooter>
          ...
          <Button key="confirm" type="primary" onClick={() => handleConfirm(brandId, colorId)}>
            Confirmar
          </Button>
        </CustomFooter>
      }
